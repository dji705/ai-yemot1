...
export interface Call {
     ...
     * @example דוגמה לשימוש בהמרת טקסט לדיבור (TTS):
     * ```typescript
     * // שימוש בסוג 'file' עם מזהה מיוחד 'tn-' לטקסט
     * await call.read([
     *   { type: 'file', data: 'tn-שלום וברוכים הבאים' } 
     * ], 'tap');
     * 
     * // שימוש בסוג 'ai' ישירות
     * await call.read([
     *   { type: 'ai', data: 'שלום וברוכים הבאים' } 
     * ], 'tap');
     * ```
     */
     ...
     * ```
     * @example שימוש בהמרת טקסט לדיבור (TTS)
     * ```js
     * // שימוש בסוג 'file' עם מזהה מיוחד 'tn-' לטקסט
     * call.id_list_message([{ type: 'file', data: 'tn-תודה על פנייתך, הפנייה התקבלה במערכת' }]);
     * 
     * // שימוש בסוג 'ai' ישירות
     * call.id_list_message([{ type: 'ai', data: 'תודה על פנייתך, הפנייה התקבלה במערכת' }]);
     * ```
     */
...
export interface Msg {
     ...
     * - music_on_hold: מוזיקת המתנה
     * - date: תאריך
     * - dateH: תאריך עברי
     * - ai: המרה אוטומטית של טקסט לדיבור (יוצר קובץ קולי)
     */
    type: 'file' | 'text' | 'speech' | 'digits' | 'number' | 'alpha' | 'zmanim' | 'go_to_folder' | 'system_message' | 'music_on_hold' | 'date' | 'dateH' | 'ai'
    /**
     * תוכן ההודעה:
     * - עבור 'file': נתיב לקובץ קולי, או טקסט המתחיל ב-'tn-' להמרה אוטומטית לדיבור
     * - עבור 'text': טקסט שיוקרא
     * - עבור 'ai': טקסט שיומר לדיבור ויישמר כקובץ
     * - עבור 'zmanim': אובייקט עם פרטי הזמנים
     * - עבור 'music_on_hold': אובייקט עם פרטי המוזיקה
     * - עבור סוגים אחרים: מחרוזת או מספר לפי הצורך
     */
    ...
    removeInvalidChars?: boolean
    /**
     * עבור 'ai': נתיב לשמירת הקובץ (ברירת מחדל: התיקייה הנוכחית)
     */
    uploadPath?: string
}
...
// Add export for ttsUpload function
export interface TtsUploadResult {
    path: string;
    [key: string]: any;
}

export function ttsUpload(text: string, uploadPath?: string): Promise<TtsUploadResult>;
export function tts(text: string): Promise<{response: ArrayBuffer, rand: string} | false>;
